...
function App() {
  ...
  const handleLogin = async (values) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        message.error(data.error || 'Erro ao logar');
      }
    } finally {
  ...
  const handleRegister = async (values) => {
    ...
    try {
      ...
      if (res.ok) {
        setUser(data);
        localStorage.setItem('pedzap_user', JSON.stringify(data));
  ...
  // Simulação de login social (frontend)
  const handleSocial = async (provider) => {
    setLoadingAuth(true);
    try {
      // Aqui normalmente você usaria o SDK do Google/Facebook
      // Simulação: dados fake
      let body;
      if (provider === 'google') {
        body = { googleId: 'fake-google-id', nome: 'Usuário Google', email: 'google@exemplo.com' };
      } else {
        body = { facebookId: 'fake-facebook-id', nome: 'Usuário Facebook', email: 'facebook@exemplo.com' };
      }
      const res = await fetch(`http://localhost:3001/api/auth/social/${provider}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        setUser(data.user);
        localStorage.setItem('pedzap_user', JSON.stringify(data.user));
        message.success('Login social realizado!');
      } else {
        message.error(data.error || 'Erro no login social');
      }
    } finally {
      setLoadingAuth(false);
    }
              ...
              items={[{
                ...
                children: (
                  <Form layout="vertical" onFinish={handleLogin} autoComplete="off">
                    <Form.Item name="email" label="E-mail" rules={[{ required: true, type: 'email', message: 'Digite um e-mail válido' }]}> <Input /> </Form.Item>
                    <Form.Item name="senha" label="Senha" rules={[{ required: true, message: 'Digite sua senha' }]}> <Input.Password /> </Form.Item>
                    <Form.Item>
                ...
                children: (
                  <Form layout="vertical" onFinish={handleRegister} autoComplete="off">
                    <Form.Item name="nome" label="Nome" rules={[{ required: true, message: 'Digite seu nome' }]}> <Input /> </Form.Item>
                    <Form.Item name="email" label="E-mail" rules={[{ required: true, type: 'email', message: 'Digite um e-mail válido' }]}> <Input /> </Form.Item>
                    <Form.Item name="senha" label="Senha" rules={[{ required: true, min: 6, message: 'Mínimo 6 caracteres' }]}> <Input.Password /> </Form.Item>
                    <Form.Item>
                      <Button type="primary" htmlType="submit" loading={loadingAuth} block>Criar Conta</Button>
                    </Form.Item>
                  </Form>
                ),
              }]}
